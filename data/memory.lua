...
function getGameInfo() -- Reads game memory to determine which game is running
  local version, lan, gen, sel
	if string.find(bts(memory.readbyterange(0x0134, 12),12), "POKEMON")
    or string.find(bts(memory.readbyterange(0x0134, 12),12), "PM") then
  ...
  elseif string.find(bts(memory.readbyterange(0x080000A0, 12),12), "POKEMON")
    or string.find(bts(memory.readbyterange(0x080000A0, 12),12), "PKMN") then
...
function checkLast(pid,checksum,start,gen)
...
function fetchPokemon(start)
  -- Fetches Pokemon info from memory and returns a table with all the data
  local mdword=memory.readdwordunsigned
  local mword=memory.readwordunsigned
  local mbyte=memory.readbyteunsigned
